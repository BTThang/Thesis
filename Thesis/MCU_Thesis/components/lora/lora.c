...
void lora_read_reg_buffer(int reg, uint8_t* val, int len) {
    ...
    spi_transaction_t t = {
        .flags = 0, .length = 8 * (len + 1), .tx_buffer = out, .rx_buffer = in};
...
void lora_explicit_header_mode(void) {
    ...
    lora_write_reg(REG_MODEM_CONFIG_1,
                   lora_read_reg(REG_MODEM_CONFIG_1) & 0xfe);
}
...
void lora_implicit_header_mode(int size) {
    __implicit = 1;
    lora_write_reg(REG_MODEM_CONFIG_1,
                   lora_read_reg(REG_MODEM_CONFIG_1) | 0x01);
...
void lora_set_bandwidth(int sbw) {
    if (sbw < 10) {
        lora_write_reg(REG_MODEM_CONFIG_1,
                       (lora_read_reg(REG_MODEM_CONFIG_1) & 0x0f) | (sbw << 4));
    }
}
...
void lora_enable_crc(void) {
    lora_write_reg(REG_MODEM_CONFIG_2,
                   lora_read_reg(REG_MODEM_CONFIG_2) | 0x04);
}
...
void lora_disable_crc(void) {
    lora_write_reg(REG_MODEM_CONFIG_2,
                   lora_read_reg(REG_MODEM_CONFIG_2) & 0xfb);
}
...
int lora_receive_packet(uint8_t* buf, int size) {
     ...
     */
            ...
            // lora_write_reg(REG_FIFO_ADDR_PTR, FIFO_EMPTY);
