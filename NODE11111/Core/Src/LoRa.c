...
void lora_addr_send(LoRa *_LoRa, lora_send_msg_t send_msg) {
	uint8_t *send_buf = (uint8_t*) malloc(24 + send_msg.buf_size);

	memcpy(send_buf, send_msg.src_addr, 12);
	memcpy(send_buf + 12, send_msg.dest_addr, 12);
	memcpy(send_buf + 24, send_msg.buf, send_msg.buf_size);
//	lora_send_packet(send_buf, 24 + send_msg.buf_size);
