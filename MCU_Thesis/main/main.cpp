...
void setClock(void* pvParameters) {
    ...
    // if (!obtain_time()) {
    //     ESP_LOGE(pcTaskGetTaskName(0), "Fail to getting time over
    //     NTP."); while (1) {
    //         vTaskDelay(1);
    //     }
    // }
    ...
    while (1) {
        if (xLoraData != NULL) {
            if (xSemaphoreTake(xLoraData, portMAX_DELAY) == pdTRUE) {
                ...
                if (S1_Lamp == 1) {
                    send_lora[0] = 1;
                    lora_send_packet((uint8_t*)send_lora, 3);
                    status_relay.relay2 = true;
                } else if (S1_Lamp == 0) {
                    send_lora[0] = 2;
                    lora_send_packet((uint8_t*)send_lora, 3);
                    status_relay.relay1 = false;
                }
...
void update_dataFirebase(void* pvParameters) {
    while (1) {
        ...
        vTaskDelay(300000 / portTICK_RATE_MS);  // 5phut
    }
}
...
static void http_get_task(void *pvParameters)
{
    ...
    while(1) {
        ...
        freeaddrinfo(res);
        sprintf(REQUEST, "GET http://api.thingspeak.com/update.json?api_key=9C4L9E6D45DL1BGD&field1=%d&field2=%d&field3=%d&field4=%d\r\n", 46, 6, 29, 9);
...
void app_main(void) {
    ...
    // xTaskCreate(&getLoRa, "getLORA", 1024 * 4, NULL, 7, NULL);
    // xTaskCreate(&update_dataFirebase, "updata", 1024 * 8, NULL, 8, NULL);
